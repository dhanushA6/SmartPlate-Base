...
class NalamRetriever:
    def __init__(
        ...
        # 3️⃣ Get (or create) collection.
        # `get_collection` throws if it doesn't exist yet, so we fall back to create.
        try:
            ...
            print(
                f"⚠️ [Retriever] Collection '{collection_name}' not found. Creating it now..."
            )
        ...
        # 4️⃣ If the collection is empty, build it from local JSON (if available).
        if auto_ingest_if_empty:
            ...
            if count == 0:
                ...
                is_default_target = (
                    db_path == "./nalam_chroma_db" and collection_name == "nalam_knowledge"
                )
                ...
                if os.path.exists(data_file) and is_default_target:
                    ...
                    try:
                        # Uses the same default paths/names as this project.
                        from data_ingestion import process_and_ingest

                        process_and_ingest()

                        # Refresh handle after ingestion.
                        self.collection = self.client.get_collection(
                            name=collection_name,
                            embedding_function=self.embedding_func,
                        )
                        ...
                        print(
                            f"⚠️ [Retriever] Auto-ingestion failed: {e}. You can run: python data_ingestion.py"
                        )
                elif os.path.exists(data_file) and not is_default_target:
                    print(
                        f"⚠️ [Retriever] Collection '{collection_name}' is empty, but auto-ingestion is only enabled for the default target "
                        f"(db_path='./nalam_chroma_db', collection_name='nalam_knowledge'). Run ingestion manually for db_path='{db_path}'."
                    )
                else:
                    print(
                        f"⚠️ [Retriever] Collection '{collection_name}' is empty and '{data_file}' was not found. Run: python data_ingestion.py"
                    )
