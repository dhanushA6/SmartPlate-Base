...
class FoodNutrients:
    ...
    selenium_ug: Optional[float] = None   # antioxidant (ug)
    ...
    # - Vitamins ------------------------------------------------------------
    vita_ug: Optional[float] = None   # Vitamin A (RAE, ug)
    vite_mg: Optional[float] = None   # Vitamin E
    vitd2_ug: Optional[float] = None   # Ergocalciferol (ug)
    vitd3_ug: Optional[float] = None   # Cholecalciferol (ug)
    vitk1_ug: Optional[float] = None   # Phylloquinone
    vitk2_ug: Optional[float] = None   # Menaquinone
    folate_ug: Optional[float] = None   # Total folate (ug)
    ...
    vitb7_ug: Optional[float] = None   # Biotin (ug)
    vitb9_ug: Optional[float] = None   # Folic acid (ug)
    vitc_mg: Optional[float] = None   # Ascorbic acid
    carotenoids_ug: Optional[float] = None   # Total carotenoids (ug)
...
class FoodRules:
    ...
    def rule_sodium(n: FoodNutrients) -> RuleResult:
        """
        High sodium is hypertensive; T2D patients have 2x higher hypertension prevalence.
        Threshold (per portion):
          <= 120 mg    -> Suitable   (+1)
...
class MicronutrientRules:
    ...
    THRESHOLDS: Dict[str, Tuple[str, float, str]] = {
        ...
        "selenium_ug": ("Selenium", 10.0, "ug"),
        "calcium_mg": ("Calcium", 80.0, "mg"),
        "iron_mg": ("Iron", 2.0, "mg"),
        "vite_mg": ("Vitamin E", 2.0, "mg"),
        "vita_ug": ("Vitamin A", 50.0, "ug"),
        "carotenoids_ug": ("Carotenoids", 500.0, "ug"),
        "folate_ug": ("Folate", 50.0, "ug"),
        "vitb6_mg": ("Vitamin B6", 0.3, "mg"),
        "vitb9_ug": ("Vitamin B9 (Folic Acid)", 50.0, "ug"),
        "vitd2_ug": ("Vitamin D2", 1.0, "ug"),
        "vitd3_ug": ("Vitamin D3", 1.0, "ug"),
        "vitb1_mg": ("Vitamin B1 (Thiamine)", 0.2, "mg"),
        "vitb2_mg": ("Vitamin B2 (Riboflavin)", 0.2, "mg"),
        "vitb3_mg": ("Vitamin B3 (Niacin)", 2.0, "mg"),
        "vitb7_ug": ("Vitamin B7 (Biotin)", 5.0, "ug"),
...
class PatientConditionRules:
    ...
    def rule_ckd_protein(n: FoodNutrients, patient: PatientProfile) -> Optional[RuleResult]:
        ...
        v = n.protein_g
        protein_limit_g = 0.6 * patient.weight_kg / 3
...
class FoodSuitabilityEngine:
    ...
    def _evaluate_food(self, food: FoodItem) -> Dict:
        ...
        energy_rule_input.energy_kcal = raw.energy_kcal
...
def print_summary(result: Dict) -> None:
    ...
    for f in result["foods"]:
        icon = {"Suitable": "[OK]", "Moderate": "[~]", "Not Suitable": "[X]"}.get(f["suitability"], "[?]")
    ...
    icon = {"Suitable": "OK SUITABLE", "Moderate": "~ MODERATE", "Not Suitable": "X NOT SUITABLE"}.get(final, final)
...
if __name__ == "__main__":
    # Alternate realistic test input (different from the sample helpers above)
